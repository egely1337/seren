...
typedef struct kernel_font {
    ...
    int (*get_glyph)(struct kernel_font *font, unsigned char c, font_glyph_t *out_glyph);
 ...
 */
int font_init_psf(kernel_font_t *font, const void *psf_binary_start, size_t psf_binary_size);

static inline int font_get_glyph(kernel_font_t *font, unsigned char c, font_glyph_t *out_glyph) {
    if (font && font->get_glyph) return font->get_glyph(font, c, out_glyph);
    return -1;
}
