...
typedef struct gfx_device {
    ...
    void (*put_pixel)(struct gfx_device *dev, uint64_t x, uint64_t y, uint32_t color);
...
static inline void gfx_put_pixel(gfx_device_t *dev, uint64_t x, uint64_t y, uint32_t color) {
    if (dev && dev->put_pixel) dev->put_pixel(dev, x, y, color);
}

static inline void gfx_clear_screen(gfx_device_t *dev, uint32_t color) {
    if (dev && dev->clear_screen) dev->clear_screen(dev, color);
}
