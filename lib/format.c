...
static void append_char(char **buf_ptr, char c, size_t *remaining_size, int *written_count) {
...
static void append_string(char **buf_ptr, const char *s, size_t *remaining_size, int *written_count) {
    if (!s) s = "(null)";
    ...
    while (*fmt) {
        ...
        switch (*fmt) {
            ...
            case 'p': {
                ...
                if (min_width > 0 && min_width > ptr_min_width) ptr_min_width = min_width;

                itoa_k(ptr_val, temp_num_buf, 16, 0, ptr_min_width, '0'); // Pad pointers with '0'
                append_string(&buf, temp_num_buf, &remaining_size, &written_count);
                break;
            }
            case '%': {
...
ensure_null_term:
    ...
    // and size was 1, buf_start[0] was written by append_char if any char was processed.
    // If size was 0, kvsnprintf returned -1.
    // If size was 1, and something was written, buf_start[0] has that char, it should be '\0'.
    // The append_char/string already ensure remaining_size > 1 for writing.
    // So if remaining_size becomes 1, *buf is where '\0' goes.
