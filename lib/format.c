...
char *itoa_k(long long value, char *str, int base, int is_signed, int min_width,
	...
	do {
		...
		u_value /= base;
		*ptr++ =
		    "0123456789abcdefghijklmnopqrstuvwxyz"[tmp_value % base];
		len++;
	} while (u_value);
...
int kvsnprintf(char *buf, size_t size, const char *fmt, va_list args) {
	...
	while (*fmt) {
		if (*fmt != '%') {
			append_char(&buf, *fmt, &remaining_size,
				    &written_count);
			fmt++;
			continue;
		}
		...
		char temp_num_buf[22]; // For 64-bit numbers in binary + sign +
				       // null

		switch (*fmt) {
		case 'c': {
			char c_val = (char)va_arg(args, int);
			append_char(&buf, c_val, &remaining_size,
				    &written_count);
			break;
		}
		case 's': {
			const char *s_val = va_arg(args, const char *);
			append_string(&buf, s_val, &remaining_size,
				      &written_count);
			break;
		}
		case 'd':
		case 'i': {
			...
			append_string(&buf, temp_num_buf, &remaining_size,
				      &written_count);
			break;
		}
		case 'u': {
		...
		case 'p': {
			...
			if (remaining_size > 3) {
				append_char(&buf, '0', &remaining_size,
					    &written_count);
				append_char(&buf, 'x', &remaining_size,
					    &written_count);
			} else {
				written_count += 2;
			}
...
ensure_null_term:
	...
	// and size was 1, buf_start[0] was written by append_char if any char
	// was processed. If size was 0, kvsnprintf returned -1. If size was 1,
	// and something was written, buf_start[0] has that char, it should be
	// '\0'. The append_char/string already ensure remaining_size > 1 for
	// writing. So if remaining_size becomes 1, *buf is where '\0' goes.
