...
void idt_init(void) {
    ...
    idt_set_gate(3,  (uint64_t)isr3,  KERNEL_CS, gate_attributes, 0); // Breakpoint
    idt_set_gate(4,  (uint64_t)isr4,  KERNEL_CS, gate_attributes, 0); // Overflow
    idt_set_gate(5,  (uint64_t)isr5,  KERNEL_CS, gate_attributes, 0); // Bound Range Exceeded
    idt_set_gate(6,  (uint64_t)isr6,  KERNEL_CS, gate_attributes, 0); // Invalid Opcode
    idt_set_gate(7,  (uint64_t)isr7,  KERNEL_CS, gate_attributes, 0); // Device Not Available
    idt_set_gate(8,  (uint64_t)isr8,  KERNEL_CS, gate_attributes, 0); // Double Fault (Consider using IST for this one)
    idt_set_gate(9,  (uint64_t)isr9,  KERNEL_CS, gate_attributes, 0); // Coprocessor Segment Overrun
    idt_set_gate(10, (uint64_t)isr10, KERNEL_CS, gate_attributes, 0); // Invalid TSS
    idt_set_gate(11, (uint64_t)isr11, KERNEL_CS, gate_attributes, 0); // Segment Not Present
    idt_set_gate(12, (uint64_t)isr12, KERNEL_CS, gate_attributes, 0); // Stack-Segment Fault
    idt_set_gate(13, (uint64_t)isr13, KERNEL_CS, gate_attributes, 0); // General Protection Fault
    idt_set_gate(14, (uint64_t)isr14, KERNEL_CS, gate_attributes, 0); // Page Fault
    // ISR 15 is reserved
    idt_set_gate(16, (uint64_t)isr16, KERNEL_CS, gate_attributes, 0); // x87 Floating-Point
    idt_set_gate(17, (uint64_t)isr17, KERNEL_CS, gate_attributes, 0); // Alignment Check
