...
static int psf_font_interface_get_glyph(kernel_font_t *font, unsigned char c,
	...
	return psf_get_glyph_from_internal_data(psf_internal_data, c,
						out_glyph);
}
...
int psf_font_load(void) {
	const unsigned char *font_data_start_ptr =
	    (const unsigned char *)&_binary_font_psf_start;
	const unsigned char *font_data_end_ptr =
	    (const unsigned char *)&_binary_font_psf_end;
	...
	current_font_glyph_data =
	    font_data_start_ptr + sizeof(struct psf1_header);
	...
	// TODO: If PSF1_MODEHASTAB is set, handle the Unicode sequence table
	// here. For now, we assume direct mapping of first 256/512 glyphs to
	// char codes.
