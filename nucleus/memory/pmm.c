...
static uint8_t *page_bitmap = NULL;             // Our big map of all pages: 0 for free, 1 for used
static uint64_t total_pages_managed = 0;        // How many pages in total we are keeping an eye
static uint64_t highest_phys_addr_managed = 0;  // The top physical address our bitmap covers
static uint64_t free_pages_count = 0;           // How many pages are currently free for allocation
...
static void bitmap_set_page(uint64_t page_index) {
    if (!page_bitmap || page_index >= total_pages_managed) return;
...
static int bitmap_is_page_used(uint64_t page_index) {
    if (page_index >= total_pages_managed) return 1;
    uint64_t byte_index = page_index / 8;
    uint8_t bit_index = page_index % 8;
 ...
 * @return The starting page index of the free block, or -1 if no such block is found.
 */
static int64_t bitmap_find_first_free_block(size_t num_pages) {
    if (num_pages == 0 || !page_bitmap) return -1;
 ...
 * This is the big one. It figures out available memory, sets up our tracking bitmap,
 * and reserves space for the kernel and the bitmap itself.
 */
void pmm_init(volatile struct limine_memmap_request *memmap_request) {
    if (!memmap_request || !memmap_request->response) {
        printk(KERN_EMERG "PMM: CRITICAL - Limine memmap request or response is NULL! Halting.\n");
        while(1) __asm__ volatile ("hlt"); 
    }
    if (!hhdm_request.response) {
        printk(KERN_EMERG "PMM: CRITICAL - Limine HHDM response is NULL! Halting.\n");
        while(1) __asm__ volatile ("hlt"); 
    }
    ...
    if (highest_phys_addr_managed == 0) {
        printk(KERN_EMERG "PMM: CRITICAL - No usable or bootloader-reclaimable memory found!\n");
        while(1) __asm__ volatile ("hlt");
    }
    highest_phys_addr_managed = (highest_phys_addr_managed + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
    total_pages_managed = highest_phys_addr_managed / PAGE_SIZE;
    if (total_pages_managed == 0 && highest_phys_addr_managed > 0) total_pages_managed = 1;
    ...
    for (uint64_t i = 0; i < memmap->entry_count; i++) {
        ...
        if (entry->type == LIMINE_MEMMAP_KERNEL_AND_MODULES) {
            if (entry->base <= kernel_phys_start && (entry->base + entry->length) > kernel_phys_start) {
                 kernel_phys_end = entry->base + entry->length;
                 printk(KERN_DEBUG "PMM: Kernel/Modules region from memmap: base=0x%p, length=0x%x\n", (void*)entry->base, entry->length);
                 break; 
            }
        }
    }
    if (kernel_phys_end == 0) {
        ...
        kernel_phys_end = KERNEL_PHYSICAL_LOAD_ADDR + (kernel_virtual_end_addr - KERNEL_VIRTUAL_BASE);
        printk(KERN_DEBUG "PMM: Kernel/Modules region not definitive, kernel end calculated from _kernel_end.\n");
    }
    kernel_phys_end = (kernel_phys_end + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1); 
    printk(KERN_DEBUG "PMM: Kernel physical footprint ends at (aligned): 0x%p\n", (void*)kernel_phys_end);
    ...
    for (uint64_t i = 0; i < memmap->entry_count; i++) {
        ...
        if (entry->type == LIMINE_MEMMAP_USABLE) {
            ...
            uint64_t potential_start = (kernel_phys_end > current_region_start) ? kernel_phys_end : current_region_start;
            potential_start = (potential_start + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1); 

            if (potential_start < current_region_end && (current_region_end - potential_start) >= bitmap_size_bytes) {
    ...
    if (bitmap_physical_addr == 0) {
        printk(KERN_EMERG "PMM: CRITICAL - Failed to find space for PMM bitmap!\n");
        while(1) __asm__ volatile ("hlt");
    }
    page_bitmap = (uint8_t*)(hhdm_offset + bitmap_physical_addr);
    printk(KERN_INFO "PMM: Page bitmap will be at VMA 0x%p (maps to PMA 0x%p)\n",
    ...
    for (uint64_t page_idx = 0; page_idx < total_pages_managed; page_idx++) {
        ...
        for (uint64_t k = 0; k < memmap->entry_count; k++) {
            ...
            if (current_phys_addr >= entry->base && current_phys_addr < (entry->base + entry->length)) {
                if (entry->type == LIMINE_MEMMAP_USABLE || entry->type == LIMINE_MEMMAP_BOOTLOADER_RECLAIMABLE) {
    ...
    uint64_t kernel_num_pages = (kernel_phys_end - kernel_phys_start) / PAGE_SIZE;
    printk(KERN_DEBUG "PMM: Reserving kernel pages: start_idx=%lu, num_pages=%lu\n", kernel_start_page_idx, kernel_num_pages);
    ...
    uint64_t bitmap_num_pages_val = (bitmap_size_bytes + PAGE_SIZE - 1) / PAGE_SIZE;
    printk(KERN_DEBUG "PMM: Reserving PMM bitmap pages: start_idx=%lu, num_pages=%lu\n", bitmap_start_page_idx, bitmap_num_pages_val);
    ...
    printk(KERN_INFO "PMM: Total managed: %lu MiB (%lu pages)\n",
           (total_pages_managed * PAGE_SIZE) / (1024 * 1024), total_pages_managed);
...
void* pmm_alloc_contiguous_pages(size_t num_pages) {
    if (page_bitmap == NULL || num_pages == 0) {
        printk(KERN_WARN "PMM: Alloc failed - PMM not initialized or zero pages requested.\n");
        return NULL;
    }
    ...
    // PMM returns physical addresses. Caller (VMM) maps them to virtual if needed.
    return allocated_address;
}

void pmm_free_page(void *p_addr) {
   pmm_free_contiguous_pages(p_addr, 1);
}
...
void pmm_free_contiguous_pages(void *p_addr, size_t num_pages) {
    ...
    if (physical_address % PAGE_SIZE != 0) {
        printk(KERN_WARN "PMM: Attempted to free unaligned physical address %p\n", p_addr);
        return;
    }
    ...
    for (size_t i = 0; i < num_pages; i++) {
        ...
        if (current_page_index >= total_pages_managed) {
            printk(KERN_WARN "PMM: Attempted to free page index %u out of bounds (%u).\n", current_page_index, total_pages_managed);
            break; 
        }
...
uint64_t pmm_get_total_memory(void) {
    return total_pages_managed * PAGE_SIZE;
}
